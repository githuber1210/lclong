<template>
  <div>
    <div>
        <el-input v-model="tableAbout.listQuery.tagName" placeholder="请输入标签名称" style="width: 500px;"></el-input>
        <el-button style="margin-left:14px;"  @click="search">搜索</el-button>
        <el-button style="margin-left: 10px;" type="primary"  @click="handleCreate">新增</el-button>
    </div>
      <el-table ref="multipleTable"
                :key="tableAbout.tableKey"
                :data="tableAbout.tableData"
                border
                fit
                highlight-current-row
                class="normal-table">
        <el-table-column label="编号" prop="postTagId" width="80px" align="center" />
        <el-table-column label="标签名称"  width="200px">
    ...
    <div>
      <el-pagination
          ...
          :current-page="tableAbout.listQuery.page"
          background
          :page-size="tableAbout.listQuery.pageSize"
    ...
    <el-dialog :title="editDialog.title[editDialog.status]" width="500px" :visible.sync="editDialog.visible">
      <el-form ref="editInfoForm" :model="editDataModel" label-position="right" label-width="80px">
  ...
  methods: {
    ...
    getList() {
      getTagList(this.tableAbout.listQuery).then((res) => {
        this.tableAbout.tableData = res.data.items

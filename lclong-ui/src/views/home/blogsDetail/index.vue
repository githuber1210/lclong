<template>
  <div style="color: #666">
    <div style="margin: 20px 0; ">
      <div style="font-size: 20px; color: #3F5EFB; cursor: pointer">{{ blog.postTitle }}</div>
      <div style="font-size: 14px; margin-top: 10px">
        <i class="el-icon-user-solid"></i> <span>{{ blog.username }}</span>
        <i class="el-icon-time" style="margin-left: 10px"></i> <span>{{ blog.postModified}}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    load() {
      getById({postsId: this.id}).then(res => {
        this.blog = res.data
      })
    },
    loadComment() {
